...
document.addEventListener("DOMContentLoaded", () => {
	...
	function applyTheme(theme) {
		...
		if (hljsLink) {
			hljsLink.href = DARK_THEMES.has(theme)
		...
		document
			.querySelectorAll(".theme-option, .mobile-theme-option")
			.forEach((btn) => {
	...
	mobileThemeToggle.addEventListener("click", (e) => {
		e.stopPropagation();
		const open = mobileThemeList.style.display === "block";
		mobileThemeList.style.display = open ? "none" : "block";
	});
	mobileThemeList.querySelectorAll(".mobile-theme-option").forEach((btn) => {
		btn.addEventListener("click", function () {
			applyTheme(this.dataset.theme);
			mobileThemeList.style.display = "none";
		});
	});
...
**Created with ❤️ for markdown enthusiasts**`;
	...
	mobileThemeToggle.addEventListener("click", () => {
		mobileThemeList.style.display =
			mobileThemeList.style.display === "block" ? "none" : "block";
	});
	...
	function sanitizePdfElementForCanvas(container, theme) {
		...
		nodes.forEach((el) => {
			...
			if (
				typeof cs.textShadow === "string" &&
				cs.textShadow.includes("color(")
			) {
				el.style.textShadow = "none";
			}
			if (
				typeof cs.backgroundImage === "string" &&
				cs.backgroundImage.includes("color(")
			) {
			...
			if (
				typeof cs.backdropFilter === "string" &&
	...
	exportHtml.addEventListener("click", (event) => {
		...
		try {
			...
			const isDarkTheme = DARK_THEMES.has(
...
<body>
  <article class="markdown-body">
	...
	function analyzeGraphicsForPageBreaks(tempElement) {
		try {
			...
			console.log(
				...
				graphics.length,
				graphics.map((g) => g.type),
			);

			// Step 2: Calculate positions for each element
			const elementsWithPositions = calculateElementPositions(
				graphics,
				tempElement,
			);
			console.log(
			...
			const { boundaries: pageBoundaries, pageHeightPx } =
				calculatePageBoundaries(totalHeight, elementWidth, PAGE_CONFIG);
			...
			const splitElements = detectSplitElements(
	 ...
	 */
	function applyPageBreaksWithCascade(
		tempElement,
		pageConfig,
		maxIterations = 10,
	) {
		...
		if (iteration >= maxIterations) {
			console.warn(
				"Page-break stabilization reached max iterations:",
				maxIterations,
			);
		}
		...
		console.log("Page-break cascade complete:", {
			...
			oversizedCount: analysis.oversizedElements
				? analysis.oversizedElements.length
				: 0,
		});
	...
	exportPdf.addEventListener("click", async (event) => {
		event.preventDefault();
		...
		try {
			...
			tempElement.style.backgroundColor = isDark ? "#0f141c" : "#fcf6ee";
			tempElement.style.color = isDark ? "#e5e7eb" : "#1c1f26";
			...
			if (window.MathJax) {
				...
				const assistiveElements =
					tempElement.querySelectorAll("mjx-assistive-mml");
				assistiveElements.forEach((el) => el.remove());

				const mathScripts = tempElement.querySelectorAll(
					'script[type*="math"], script[type*="tex"]',
				);
			...
			const pageBreakAnalysis = applyPageBreaksWithCascade(
				tempElement,
				PAGE_CONFIG,
			);
			if (
				pageBreakAnalysis.oversizedElements &&
				pageBreakAnalysis.pageHeightPx
			) {
				handleOversizedElements(
					pageBreakAnalysis.oversizedElements,
					pageBreakAnalysis.pageHeightPx,
				);
			}
			...
			const canvas = await html2canvas(tempElement, {
				...
				allowTaint: true,
				logging: false,
				backgroundColor: isDark ? "#0f141c" : "#fcf6ee",
	...
	closeDropzoneBtn.addEventListener("click", (e) => {
		e.stopPropagation();
		dropzone.style.display = "none";
	});
